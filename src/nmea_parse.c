...
processGPGSV (char *sentence)
{
	...
	while (n < gNMEAdata.in_view && n < m)
	{
		...
		update_field_i (sentence, f++, &gNMEAdata.elevation[n],
				C_SAT);
...
static void
do_lat_lon (char *sentence, int begin)
{
	double lat, lon, d, m;
	char str[20], *p;
	int updated = 0;
...
static void
update_field_i (char *sentence, int fld, int *dest, int mask)
{
	int tmp;
...
#if 0
static void
update_field_f (char *sentence, int fld, double *dest, int mask)
{
	double tmp;
...
checksum (char *sentence)
{
	...
	{
		fprintf (stderr,
			 "checksum error: checksum not preceded with '*'\n");
		return 0;
	}
	...
	{
		fprintf (stderr,
			 "checksum error: NMEA: \"%s\", read as: %#04X, calculated as: %#04X\n",
