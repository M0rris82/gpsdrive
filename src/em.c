...
putlong (char *dm, int sign)
{
	...
	rad = (floor (tmpl / 100) +
	       (fmod (tmpl, 100.0) / 60)) * 100000000 * PI / 180;
...
em_init ()
{
	...
	if (latitude && longitude)
	{
		...
		data[10] = tm->tm_sec;
		*(long *) (data + 11) =
			putlong (latitude, (latd == 'S') ? 1 : 0);
		*(long *) (data + 13) =
			putlong (longitude, (lond == 'W') ? 1 : 0);
		data[15] = data[16] = 0;
...
handle1000 (unsigned short *p)
{
	...
	fprintf (stderr, "date: %d %d %d  %d:%d:%d\n",
		 p[O (19)], p[O (20)], p[O (21)], p[O (22)], p[O (23)],
		 p[O (24)]);
...
handle1002 (unsigned short *p)
{
	...
	for (i = 0; i < 12; i++)
	{
		...
		fprintf (stderr, " used:%d",
			 (p[O (15 + (3 * i))] & 1) ? 1 : 0);
		fprintf (stderr, " eph:%d",
			 (p[O (15 + (3 * i))] & 2) ? 1 : 0);
		fprintf (stderr, " val:%d",
			 (p[O (15 + (3 * i))] & 4) ? 1 : 0);
		fprintf (stderr, " dgps:%d",
			 (p[O (15 + (3 * i))] & 8) ? 1 : 0);
...
handle1003 (unsigned short *p)
{
	...
	for (j = 0; j < 12; j++)
	{
		if (j < gNMEAdata.in_view)
		{
			...
			gNMEAdata.azimuth[j] =
				p[O (16 + (3 * j))] * 180 / (PI * 10000);
			gNMEAdata.elevation[j] =
				p[O (17 + (3 * j))] * 180 / (PI * 10000);
#if 0
			fprintf (stderr, "Sat%02d:", i);
...
handle1005 (unsigned short *p)
{
	...
	for (i = 0; i < numcorrections; i++)
	{
		...
		fprintf (stderr, "ephemeris:%d",
			 (p[O (13 + i)] & 64) ? 1 : 0);
		fprintf (stderr, "rtcm corrections:%d",
			 (p[O (13 + i)] & 128) ? 1 : 0);
		fprintf (stderr, "rtcm udre:%d",
			 (p[O (13 + i)] & 256) ? 1 : 0);
		fprintf (stderr, "sat health:%d",
			 (p[O (13 + i)] & 512) ? 1 : 0);
		fprintf (stderr, "rtcm sat health:%d",
			 (p[O (13 + i)] & 1024) ? 1 : 0);
		fprintf (stderr, "corrections state:%d",
			 (p[O (13 + i)] & 2048) ? 1 : 0);
		fprintf (stderr, "iode mismatch:%d",
			 (p[O (13 + i)] & 4096) ? 1 : 0);
	}
#endif
}
	 ...
	 fd_set * nmea_fds)
{
	...
	if (p[h->ndata] == em_checksum (p, h->ndata))
	{
		...
		case 1000:
			...
			if (gNMEAdata.mode > 1)
			{
				sprintf (bufp,
					 ...
					 gNMEAdata.hours, gNMEAdata.minutes,
					 gNMEAdata.seconds,
					 ...
					 ((gNMEAdata.longitude >
					   0) ? 'E' : 'W'), gNMEAdata.status,
					 gNMEAdata.satellites, gNMEAdata.hdop,
					 gNMEAdata.altitude, 'M',
					 gNMEAdata.separation, 'M', "", "");
				add_checksum (bufp + 1);
			...
			sprintf (bufp,
				 ...
				 gNMEAdata.speed, gNMEAdata.course,
				 gNMEAdata.day, gNMEAdata.month,
		...
		case 1002:
			...
			bufp = bufp + strlen (bufp);
			sprintf (bufp, "%.2f,%.2f,%.2f*", gNMEAdata.pdop,
				 gNMEAdata.hdop, gNMEAdata.vdop);
			...
			for (i = 0; i < 12; i++)
			{
				sprintf (bufp, ",%02d,%X", gNMEAdata.Zs[i],
					 gNMEAdata.Zv[i]);
				bufp = bufp + strlen (bufp);
			}
			sprintf (bufp, "*");
		...
		case 1003:
			handle1003 (p);
			bufp2 = bufp = buf;
			j = (gNMEAdata.in_view / 4) +
				(((gNMEAdata.in_view % 4) > 0) ? 1 : 0);
			while (i < 12)
			{
				if (i % 4 == 0)
					sprintf (bufp, "$GPGSV,%d,%d,%02d", j,
						 (i / 4) + 1,
						 gNMEAdata.in_view);
				bufp += strlen (bufp);
