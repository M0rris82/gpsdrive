...
modulesetup ()
{
	...
	gtk_container_set_border_width (GTK_CONTAINER (setupentry[myplace]),
					5 * PADDING);
	...
	if (flymode)
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (flymodebt),
					      TRUE);

	gtk_signal_connect (GTK_OBJECT (flymodebt),
			    "clicked", GTK_SIGNAL_FUNC (flymode_cb),
			    (gpointer) 1);

	vfr1 = gtk_radio_button_new_with_label (NULL, _("Use VFR"));
	gtk_signal_connect (GTK_OBJECT (vfr1),
			    "clicked", GTK_SIGNAL_FUNC (vfr_cb),
			    (gpointer) 1);

	vfr2 = gtk_radio_button_new_with_label (gtk_radio_button_group
						(GTK_RADIO_BUTTON (vfr1)),
						_("Use IFR"));
	...
	disdevbt =
		gtk_check_button_new_with_label (_
						 ("disable vert. deviation warning above 5000ft MSL"));
	if (disdevwarn)
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (disdevbt),
					      TRUE);

	gtk_signal_connect (GTK_OBJECT (disdevbt),
			    "clicked", GTK_SIGNAL_FUNC (disdev_cb),
			    (gpointer) 1);
