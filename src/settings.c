...
"GMT-5",
	"GMT-4", "GMT-3", "GMT-2", "GMT-1", "GMT+0", "GMT+1", "GMT+2",
		"GMT+3",
	"GMT+4", "GMT+5",
...
change_font_callback (GtkWidget * button, gint data)
{
	...
	if (data == 1)
		dialog = gtk_font_selection_dialog_new (_
							("Setting WP label font"));
	if (data == 2)
		dialog = gtk_font_selection_dialog_new (_
							("Setting big display font"));
	...
	if (data == 1)
		gtk_font_selection_dialog_set_font_name
			(GTK_FONT_SELECTION_DIALOG (dialog), wplabelfont);
	if (data == 2)
		gtk_font_selection_dialog_set_font_name
			(GTK_FONT_SELECTION_DIALOG (dialog), bigfont);
	...
	if (response == GTK_RESPONSE_OK)
	{
		...
		fn = gtk_font_selection_dialog_get_font_name
...
change_color_callback (GtkWidget * button, gint data)
{
	...
	dialog = gtk_color_selection_dialog_new (_
						 ("Setting big display color"));
...
change_friendscolor_callback (GtkWidget * button, gint data)
{
	...
	{
		gtk_color_selection_get_current_color (colorsel, &color);
		g_snprintf (friendscolor, sizeof (friendscolor),
			    "#%02x%02x%02x", color.red >> 8, color.green >> 8,
			    color.blue >> 8);
		if (debug)
			fprintf (stderr, "\nnew color: %s\n", trackcolor);
		orange = color;
...
mainsetup (void)
{
	...
	GtkWidget *metric, *nautic, *garminbt, *dgpsbt, *earthmatebt,
		*minsecbt;
	GtkWidget *table2, *shadowbt, *etchbt, *drawgridbt, *s1, *s2,
		*nighttable, *label1a;
	GtkWidget *label2, *f1, *f2, *f3, *f4, *f5, *framefont, *simfollowbt;
	GtkWidget *night1, *night2, *night3, *vbox, *font1, *font2,
		*font3, *slowcpulabel, *trackcolorlabel, *trackcolorbt,
		*noserialbt;
	...
	gtk_box_pack_start (GTK_BOX (vbox), table2, FALSE, FALSE,
			    10 * PADDING);
	...
	for (i = 0; i < dircount; i++)
	{
		...
		gtk_signal_connect (GTK_OBJECT (setupfn[i]), "clicked",
				    GTK_SIGNAL_FUNC (wpfileselect_cb),
				    (gpointer) i);
		gtk_table_attach_defaults (GTK_TABLE (table2), setupfn[i],
					   i % 2, i % 2 + 1, i / 2,
					   i / 2 + 1);
	...
	gtk_container_set_border_width (GTK_CONTAINER (mainbox2),
					5 * PADDING);
	...
	if (shadow)
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (shadowbt),
					      TRUE);

	gtk_signal_connect (GTK_OBJECT (shadowbt),
			    "clicked", GTK_SIGNAL_FUNC (shadow_cb),
			    (gpointer) 1);

	etchbt = gtk_check_button_new_with_label (_("Etched frames"));
	if (etch)
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (etchbt),
					      TRUE);

	gtk_signal_connect (GTK_OBJECT (etchbt),
			    "clicked", GTK_SIGNAL_FUNC (etch_cb),
			    (gpointer) 1);

	drawgridbt = gtk_check_button_new_with_label (_("Draw grid"));
	if (drawgrid)
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (drawgridbt),
					      TRUE);

	gtk_signal_connect (GTK_OBJECT (drawgridbt),
			    "clicked", GTK_SIGNAL_FUNC (drawgrid_cb),
			    (gpointer) 1);

	simfollowbt =
		gtk_check_button_new_with_label (_
						 ("Simulation: Follow target"));
	if (simfollow)
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (simfollowbt),
					      TRUE);
	...
	gtk_signal_connect (GTK_OBJECT (GTK_COMBO (slowcpubt)->entry),
			    "changed", GTK_SIGNAL_FUNC (slowcpu_cb),
			    (gpointer) 0);
	...
	gtk_signal_connect (GTK_OBJECT (trackcolorbt), "clicked",
			    G_CALLBACK (change_trackcolor_callback),
			    (gpointer) 0);
	...
	gtk_signal_connect (GTK_OBJECT (mapdirbt),
			    "changed", GTK_SIGNAL_FUNC (mapdir_cb),
			    (gpointer) 1);


	night1 = gtk_radio_button_new_with_label (NULL, _("Automatic"));
	gtk_signal_connect (GTK_OBJECT (night1),
			    "clicked", GTK_SIGNAL_FUNC (night_cb),
			    (gpointer) 2);

	night2 = gtk_radio_button_new_with_label (gtk_radio_button_group
						  (GTK_RADIO_BUTTON (night1)),
						  _("On"));
	gtk_signal_connect (GTK_OBJECT (night2), "clicked",
			    GTK_SIGNAL_FUNC (night_cb), (gpointer) 1);
	night3 = gtk_radio_button_new_with_label (gtk_radio_button_group
						  (GTK_RADIO_BUTTON (night1)),
						  _("Off"));
	gtk_signal_connect (GTK_OBJECT (night3), "clicked",
			    GTK_SIGNAL_FUNC (night_cb), (gpointer) 0);
	if (nightmode == 0)
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (night3),
					      TRUE);
	if (nightmode == 1)
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (night2),
					      TRUE);
	if (nightmode == 2)
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (night1),
					      TRUE);

	gtk_table_attach_defaults (GTK_TABLE (misctable), shadowbt, 0, 2, 0,
				   1);
	gtk_table_attach_defaults (GTK_TABLE (misctable), simfollowbt, 0, 2,
				   1, 2);
	gtk_table_attach_defaults (GTK_TABLE (misctable), etchbt, 2, 4, 0, 1);
	gtk_table_attach_defaults (GTK_TABLE (misctable), drawgridbt, 2, 4, 0,
				   3);

	gtk_table_attach_defaults (GTK_TABLE (misctable), slowcpulabel, 0, 2,
				   3, 4);
	gtk_table_attach_defaults (GTK_TABLE (misctable), slowcpubt, 0, 2, 4,
				   5);

	gtk_table_attach_defaults (GTK_TABLE (misctable), trackcolorlabel, 2,
				   4, 3, 4);
	gtk_table_attach_defaults (GTK_TABLE (misctable), trackcolorbt, 2, 4,
				   4, 5);
	...
	gtk_box_pack_start (GTK_BOX (v4), gpstable, FALSE, FALSE,
			    1 * PADDING);
	...
	if (testgarmin)
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (garminbt),
					      TRUE);
	...
	gtk_table_attach_defaults (GTK_TABLE (gpstable), garminbt, 0, 1, 0,
				   1);


	dgpsbt = gtk_check_button_new_with_label (_("Use DGPS-IP"));
	if (usedgps)
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (dgpsbt),
					      TRUE);

	gtk_signal_connect (GTK_OBJECT (dgpsbt),
			    "clicked", GTK_SIGNAL_FUNC (usedgps_cb),
			    (gpointer) 1);
	...
	if (earthmate)
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (earthmatebt),
					      TRUE);
	...
	gtk_table_attach_defaults (GTK_TABLE (gpstable), earthmatebt, 1, 2, 0,
				   1);

	noserialbt = gtk_check_button_new_with_label (_("Use serial conn."));
	if (!disableserial)
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (noserialbt),
					      TRUE);

	gtk_signal_connect (GTK_OBJECT (noserialbt),
			    "clicked", GTK_SIGNAL_FUNC (noserial_cb),
			    (gpointer) 1);

	gtk_table_attach_defaults (GTK_TABLE (gpstable), noserialbt, 1, 2, 1,
				   2);
	...
	gtk_table_attach_defaults (GTK_TABLE (gpstable), serialbt, 0, 1, 3,
				   4);
	...
	gtk_table_attach_defaults (GTK_TABLE (gpstable), option_menu, 1, 2, 3,
				   4);
	...
	gtk_signal_connect (GTK_OBJECT (miles),
			    "clicked", GTK_SIGNAL_FUNC (miles_cb),
			    (gpointer) 1);
	metric = gtk_radio_button_new_with_label (gtk_radio_button_group
						  (GTK_RADIO_BUTTON (miles)),
						  _("Metric"));
	gtk_signal_connect (GTK_OBJECT (metric), "clicked",
			    GTK_SIGNAL_FUNC (miles_cb), (gpointer) 2);
	nautic = gtk_radio_button_new_with_label (gtk_radio_button_group
						  (GTK_RADIO_BUTTON (miles)),
						  _("Nautic"));
	gtk_signal_connect (GTK_OBJECT (nautic), "clicked",
			    GTK_SIGNAL_FUNC (miles_cb), (gpointer) 3);
	if (milesflag)
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (miles),
					      TRUE);
	if (metricflag)
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (metric),
					      TRUE);
	if (nauticflag)
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (nautic),
					      TRUE);

	minsecbt = gtk_check_button_new_with_label (_("Decimal position"));
	if (!minsecmode)
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (minsecbt),
					      TRUE);

	gtk_signal_connect (GTK_OBJECT (minsecbt),
			    "clicked", GTK_SIGNAL_FUNC (minsec_cb),
			    (gpointer) 0);
	...
	gtk_signal_connect (GTK_OBJECT (s1),
			    ...
			    (gpointer) 1);
	s2 = gtk_radio_button_new_with_label (gtk_radio_button_group
					      (GTK_RADIO_BUTTON (s1)),
					      _("Expedia USA"));
	...
	gtk_table_attach_defaults (GTK_TABLE (nighttable), night1, 0, 1, 0,
				   1);
	gtk_table_attach_defaults (GTK_TABLE (nighttable), night2, 1, 2, 0,
				   1);
	gtk_table_attach_defaults (GTK_TABLE (nighttable), night3, 2, 3, 0,
				   1);
	...
	if (sound_direction)
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (sounddirbt),
					      TRUE);
	...
	if (sound_distance)
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (sounddistbt),
					      TRUE);
	...
	if (sound_gps)
		gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (soundgpsbt),
					      TRUE);
	gtk_signal_connect (GTK_OBJECT (soundgpsbt), "clicked",
			    GTK_SIGNAL_FUNC (soundsettings_cb), (gpointer) 4);
	gtk_table_attach_defaults (GTK_TABLE (soundtable), sounddirbt, 0, 1,
				   0, 1);
	gtk_table_attach_defaults (GTK_TABLE (soundtable), sounddistbt, 0, 1,
				   1, 2);
	gtk_table_attach_defaults (GTK_TABLE (soundtable), soundspeedbt, 1, 2,
				   0, 1);
	gtk_table_attach_defaults (GTK_TABLE (soundtable), soundgpsbt, 1, 2,
				   1, 2);
	...
	gtk_box_pack_start (GTK_BOX (mainbox), framesound, FALSE, TRUE,
			    1 * PADDING);
	gtk_box_pack_start (GTK_BOX (mainbox), framefont, FALSE, TRUE,
			    1 * PADDING);
	...
	gtk_tooltips_set_tip (GTK_TOOLTIPS (tooltips), s2,
			      _("Set Expedia as default download server"),
			      NULL);
	...
	gtk_tooltips_set_tip (GTK_TOOLTIPS (tooltips), trackcolorbt,
			      _("Set here the color of the drawn track"),
			      NULL);
...
daylights (void)
{
	...
	D = st->tm_mday;
	MA = 357 + 0.9856 * Rank;
	C = 1.914 * sin (M_PI * MA / 180.0) +
		0.02 * sin (2 * M_PI * MA / 180.0);
	...
	Ho = acos (((-0.01454 -
		     sin (Dec) * sin (lat)) / (cos (Dec) * cos (lat))));
	Ho = 180.0 * Ho / M_PI;
	Ho = Ho / 15;
	...
	if (debug)
	{
		...
		g_print ("\nhour: %d:%02d, transit: %d:%02d ", (int) hour,
			 (int) ((hour - (int) hour) * 60), (int) suntransit,
			 (int) ((suntransit - (int) suntransit) * 60));
		g_print ("\nhour: %d:%02d, sunset: %d:%02d ", (int) hour,
			 (int) ((hour - (int) hour) * 60), (int) sunset,
...
infosettz (GtkWidget * widget, guint datum)
{
	...
	sc = (char *)
		gtk_entry_get_text (GTK_ENTRY (GTK_COMBO (ge12)->entry));
...
infos (void)
{
	...
	gtk_box_pack_start (GTK_BOX (mainbox), table, FALSE, FALSE,
			    10 * PADDING);
	...
	gtk_signal_connect (GTK_OBJECT (l13),
			    "clicked", GTK_SIGNAL_FUNC (storetz_cb),
			    (gpointer) 0);
	...
	gtk_entry_set_text (GTK_ENTRY (GTK_COMBO (ge12)->entry),
			    slist[zone + 12]);
...
trip (void)
{
	...
	gtk_table_attach_defaults (GTK_TABLE (table), tripunitlabel, 0, 2, 0,
				   1);
	...
	gtk_table_attach_defaults (GTK_TABLE (table), entrytripodometer, 1, 2,
				   1, 2);


	entrytriptime = gtk_entry_new ();
	gtk_entry_set_text (GTK_ENTRY (entrytriptime), "---");
	gtk_table_attach_defaults (GTK_TABLE (table), entrytriptime, 1, 2, 2,
				   3);
	...
	gtk_table_attach_defaults (GTK_TABLE (table), entryavspeed, 1, 2, 3,
				   4);

	entrymaxspeed = gtk_entry_new ();
	gtk_entry_set_text (GTK_ENTRY (entrymaxspeed), "---");
	gtk_table_attach_defaults (GTK_TABLE (table), entrymaxspeed, 1, 2, 4,
				   5);
...
friendslookup_cb (GtkWidget * widget, guint datum)
{
	...
	if ((hent != NULL) && (AF_INET == hent->h_addrtype))
	{
		...
		g_strlcpy (friendsserverip, "0.0.0.0",
...
friendssetup (void)
{
	...
	gtk_box_pack_start (GTK_BOX (mainbox), table, FALSE, TRUE,
			    3 * PADDING);
	...
	gtk_signal_connect (GTK_OBJECT (d4),
			    "changed", GTK_SIGNAL_FUNC (friendsserverfqn_cb),
			    d2);

	look = gtk_button_new_with_label (_("Lookup"));

	gtk_signal_connect (GTK_OBJECT (look),
			    "clicked", GTK_SIGNAL_FUNC (friendslookup_cb),
			    d2);
	...
	gtk_signal_connect (GTK_OBJECT (d6),
			    "changed", GTK_SIGNAL_FUNC (friendsserverip_cb),
			    d2);
	...
	gtk_table_attach_defaults (GTK_TABLE (spintable), spinner1, 0, 1, 1,
				   2);
	gtk_table_attach_defaults (GTK_TABLE (spintable), spinner2, 1, 2, 1,
				   2);
	gtk_table_attach_defaults (GTK_TABLE (spintable), spinner3, 2, 3, 1,
				   2);
...
sqlsetup (void)
{
	...
	setupentry[sqlplace] = frame =
		gtk_frame_new (_("SQL selection criterias"));
	...
	t2 = gtk_radio_button_new_with_label (gtk_radio_button_group
					      (GTK_RADIO_BUTTON (t1)),
					      _("exclude"));
	...
	gtk_box_pack_start (GTK_BOX (mainbox), scroll, TRUE, TRUE,
			    3 * PADDING);
	...
	for (i = 0; i < dbtypelistcount; i++)
	{
		...
		gtk_table_attach_defaults (GTK_TABLE (table2), l[i], 0, 1, i,
					   i + 1);
		...
		gtk_signal_connect (GTK_OBJECT (sqlfn[i]),
				    "clicked",
				    GTK_SIGNAL_FUNC (dbbuildquery_cb),
				    (gpointer) i);
		...
		gtk_table_attach_defaults (GTK_TABLE (table2), sqlfn[i], 1, 2,
					   i, i + 1);
