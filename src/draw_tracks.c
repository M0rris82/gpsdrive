...
*/
void
tracks_query_point (gdouble lat, gdouble lon, gdouble distance)
{
  gint i;
  printf ("Query Tracks: %f,%f ( %f )\n", lat, lon, distance);

  for (i = 0; i < tracks_list_count; i++)
    {
	  if ( 
	      distance >= 
	      distance_line_point(  (tracks_list + i)->lat1,(tracks_list + i)->lon1,
				    (tracks_list + i)->lat2,(tracks_list + i)->lon2,
				    lat,  lon) )
	      {
